...
Tree *tree_add_element(Tree *root, int value)
{
    ...
    while (tree1 != NULL) {
        tree2 = tree1;
        if (value < tree1->data) {
            tree1 = (Tree *) tree1->left;
        }
        else if (value > tree1->data) {
            tree1 = (Tree *) tree1->right;
        }
        else {
    ...
    NewTree->parent = (struct Tree *) tree2;
    NewTree->left = NULL;
    NewTree->right = NULL;
    if (value < tree2->data) {
        tree2->left = (struct Tree *) NewTree;
    } else {
        tree2->right = (struct Tree *) NewTree;
    }
    printf("Done!\n");
    return root;
}
...
{
    if (root != NULL) {
        tree_print((Tree *) root->right, n + 1);
        for (int j = 0; j < n; j++) printf("\t");
        printf("%d\n", root->data);
        tree_print((Tree *) root->left, n + 1);
    }
}
...
Tree *delete_element(Tree *root, int value)
{
    ...
    if (tree1->left == NULL && tree1->right == NULL) {
        ...
        tree2 = (Tree *) tree1->parent;
        if (tree2->left == (struct Tree*)tree1) {
            tree2->left = NULL;
        }
        else {
            tree2->right = NULL;
        ...
        tree2 = minimum((Tree *) tree1->right);
        tree1->data = tree2->data;
        tree4 = (Tree *) tree2->parent;
        if (tree4->left == (struct Tree*)tree2) {
            free(tree2);
            tree4->left = NULL;
        }
        if (tree4->right == (struct Tree*)tree2) {
            free(tree2);
            tree4->right = NULL;
        }
    }
...
Tree *search_in_tree(Tree *root, int value)
{
    ...
    if (value > root->data) {
        return search_in_tree((Tree *) root->right, value);
    } else {
        return search_in_tree((Tree *) root->left, value);
    }
}
...
void depth(Tree *root, int n) {
    ...
    if (root->left != NULL) traversal((Tree *) root->left, c, a);
    if (root->right != NULL) traversal((Tree *) root->right, c, a);
    printf("array done\n");
    int o = a[0];
